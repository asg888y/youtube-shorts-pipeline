...
def get_provider(name: str | None = None) -> str:
    ...
    raise RuntimeError(
        ...
        "  ANTHROPIC_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, MINIMAX_API_KEY\n"
        "  DASHSCOPE_API_KEY, XUNFEI_API_KEY\n"
...
def call_llm(prompt: str, provider: str | None = None, max_tokens: int = 1500) -> str:
    ...
    if provider == "claude":
        ...
        return _call_xunfei(prompt, max_tokens)
    else:
...
def _call_dashscope(prompt: str, max_tokens: int) -> str:
    """Call DashScope (Qwen) via Alibaba Cloud API."""
    import requests
    from .config import load_config
    ...
    if not api_key:
        raise RuntimeError("DASHSCOPE_API_KEY not set in config.json")
    ...
    r = requests.post(url, json=body, headers=headers, timeout=60)
    if r.status_code != 200:
        raise RuntimeError(f"DashScope API {r.status_code}: {r.text[:300]}")
...
def _call_xunfei(prompt: str, max_tokens: int) -> str:
    """Call 讯飞星火 via Maas API (OpenAI compatible endpoint)."""
    import requests
    from .config import load_config

    cfg = load_config()
    api_key = cfg.get("XUNFEI_API_KEY", "")
    if not api_key:
        raise RuntimeError("XUNFEI_API_KEY not set in config.json")
    ...
    if r.status_code != 200:
        raise RuntimeError(f"讯飞API {r.status_code}: {r.text[:300]}")

    data = r.json()
    choices = data.get("choices", [])
    if not choices:
        raise RuntimeError("讯飞返回空响应")
    return choices[0].get("message", {}).get("content", "").strip()
