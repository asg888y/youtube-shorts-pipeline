...
def cmd_draft(args):
    ...
    viral = getattr(args, "viral", False)

    viral_note = " [VIRAL MODE]" if viral else ""
    print(f"\n  Drafting: {args.news}{viral_note} [niche: {niche}, platform: {platform}]\n")
...
def main():
    ...
    p_draft.add_argument("--provider", default=None, help="LLM: claude, gemini, openai, ollama, minimax, dashscope")
    p_draft.add_argument("--keywords", default=None, help="Custom search keywords")
    p_draft.add_argument("--viral", action="store_true", help="Viral content mode")
    ...
    p_run.add_argument("--provider", default=None, help="LLM: claude, gemini, openai, ollama, minimax, dashscope")
    p_run.add_argument("--voice", default=None, help="TTS: edge, elevenlabs, say, dashscope")
    p_run.add_argument("--lang", default="en", choices=["en", "hi", "es", "pt", "de", "fr", "ja", "ko"])
    p_run.add_argument("--keywords", default=None, help="Custom search keywords")
    p_run.add_argument("--viral", action="store_true", help="Viral content mode")
    ...
    p_topics.add_argument("--hot", action="store_true", help="Fetch hot topics from Weibo/Zhihu/Toutiao")

    # hot - 快速获取今日热点
    p_hot = sub.add_parser("hot", help="Get today's hottest topic for viral content")
    p_hot.add_argument("--top", action="store_true", help="Auto-use top hot topic")
    p_hot.add_argument("--list", action="store_true", help="List all hot topics")
    p_hot.add_argument("--limit", type=int, default=20, help="Max topics to list")
    ...
    # Handle hot command
    if args.cmd == "hot":
        from .topics import print_hot_topics, get_top_hot_topic
        if getattr(args, "list", False):
            print_hot_topics(getattr(args, "limit", 20))
        elif getattr(args, "top", False):
            topic = get_top_hot_topic()
            if topic:
                print(f"\n  今日热点第一: {topic.title}")
                print(f"  来源: {topic.source}")
                print(f"\n  使用方法:")
                print(f"    python -m verticals draft --news '{topic.title}' --viral")
            else:
                print("  未获取到热点话题")
        else:
            print_hot_topics(20)
